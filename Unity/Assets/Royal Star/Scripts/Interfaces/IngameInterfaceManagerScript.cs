...
public class IngameInterfaceManagerScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(PhotonNetwork.IsMasterClient)
        {
            ...
            foreach (var ship in vaisseaux)
            {
                ...
                photonView.RPC("UpdateInterfaceRPC", PlayerNumbering.SortedPlayers[i], indice, ship.getPV(), ship.getBouclier());    
    ...
    [PunRPC]
    private void UpdateInterfaceRPC(int indice, int pv, int bouclier)
    {
        Debug.Log("Update de l'interface coté client " + indice.ToString());

        vaisseaux[indice].MiseAJourStats(pv, bouclier);
